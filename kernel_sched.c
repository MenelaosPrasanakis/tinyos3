...
//rlnode SCHED;
int COUNTER=0; // counter for yield calls
...
{
  /* Get the head of the first not empty SCHED list and return it */
  //it will schedule the thread that is highest in priority not the empty as it does in rr
...
void yield(enum SCHED_CAUSE cause)
{
  ...
  Mutex_Lock(&sched_spinlock);
  //checking if a boost of the threads is necessary
  if(COUNTER>CALLS) {
    boost_threads();
    COUNTER = 0;
  }
  //using switch to find the cause of the yield,and so to adjust the priority


  switch(cause) {
    //thread has finished its task before the end of the quantum
    case SCHED_IO:
    ...
    break;
    //thread use a quantum and hasn't finish it's task
    case SCHED_QUANTUM:
    ...
    break;
    //a thread wants to access a resource and tries to get the mutex,but it's locked by a thread with lower priority
    case SCHED_MUTEX:
    //also checking if the thread is waiting in the same queue for the second time
...
//moves threads to a higher priority queue after a number of times that yield has been called
//so that no starvation problem will occure 
void boost_threads(){
 
  for(int i = PRIORITY_QUEUES-2; i >= 0; i--){
      while(!is_rlist_empty(&SCHED[i])){
        rlnode* curTh = rlist_pop_front(&SCHED[i]); //take the head of the list in i-2
        curTh->tcb->priority++; //increase its priority
        rlist_push_back(&SCHED[i+1],curTh); //put it at the end of the list
